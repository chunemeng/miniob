...
RC CompositeTuple::find_cell(const TupleCellSpec &spec, Value &cell, bool should_dif) const
{
  RC rc = RC::NOTFOUND;
  for (const auto &tuple : tuples_) {
    rc = tuple->find_cell(spec, cell, false);
    if (OB_SUCC(rc)) {
      break;
    }
  }
...
RC CompositeTuple::find_cell_index(const TupleCellSpec &spec, int &index) const
{
  RC  rc           = RC::NOTFOUND;
  int index_offset = 0;
  for (const auto &tuple : tuples_) {
    int indd;
    rc = tuple->find_cell_index(spec, indd);
    if (OB_SUCC(rc)) {
      index = index_offset + indd;
      break;
    }
    index_offset += tuple->cell_num();
  }
  return rc;
}
