...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for (int i = 0; i < table_meta.null_field_num(); ++i) {
    index_fields.emplace_back(table->table_meta().field(table_meta.null_field_offset() + i));
  }
  ...
  stmt = new CreateIndexStmt(table, std::move(index_fields), create_index.index_name, create_index.is_unique);
  return RC::SUCCESS;
}
